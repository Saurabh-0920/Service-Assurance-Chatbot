...
async function identify_user(agent)
{
  ...
  const snap = await client.db("chatbot").collection("user_details").findOne({"account_number":account_number});
  
  if(snap==null){
	  await agent.add("Sorry The Account Number Given Was Not Found In Our Database , Kindly Re-Enter A Valid Account Number");
...
function report_issue(agent)
{
 ...
 agent.add("The Issue Reported Is : "+ val +"\n For Future Reference The Ticket Number Is: "+trouble_ticket+"\n We Will Try To Fix It As Soon As Possible");
}
